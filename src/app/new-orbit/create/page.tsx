...
export default function CreateAccount() {
    ...
    const [LocalStoredsigners, setLocalStoredSigners] = useState<{ name: string; address: string }[]>([]);
    ...
    const [formData, setFormData] = useState({
        name: '',
        network: 'BTTC',
        signers: [{ name: '', address: address as `0x${string}` | "" }, { name: '', address: "" }],
        threshold: 1
    })
    ...
    useEffect(() => {
        if (address) {
            setFormData({
                name: '',
                network: 'BTTC',
                signers: [{ name: getNameByAddress(address), address: address as `0x${string}` }, { name: '', address: "" }],
                threshold: 1
            })
        }
    }, [address])
    ...
    const handleSignerChange = (e: React.ChangeEvent<HTMLInputElement>, index: number, field: 'name' | 'address') => {
        ...
        if (field === 'address') {
            const existingSigner = LocalStoredsigners.find(signer => signer.address === e.target.value);
            if (existingSigner) {
                ...
                <>
                    <div className='flex items-center justify-start gap-4 border-b border-border-light mb-8'>
                        ...
                        <div className='flex-1 flex flex-col'>
                            <span className="text-lg md:text-xl font-bold">Enter the name of your Orbit Account</span>
                            <span className="text-gray-400 text-sm md:text-md mb-4">This name helps identify and manage your wallet and will be stored off-chain.</span>
                        </div>
                    </div>
            ...
            return (
                <>
                    <div className='flex items-center justify-start gap-4 border-b border-border-light mb-8'>
                        ...
                        <div className='flex-1 flex flex-col'>
                            <span className="text-lg md:text-xl font-bold">Signers and confirmations</span>
                            <span className="text-gray-400 text-sm md:text-md mb-4">Set the signer wallets of your Orbit Account and how many need to confirm to execute a valid transaction.</span>
                        </div>
                    </div>
                    <div className='pb-6 border-b border-border-light px-0 md:px-8'>
                                ...
                                <div key={index} className="flex space-x-2 items-end">
                                    ...
                                    <div className='flex-1'>
                                        ...
                                        <input
                                            ...
                                            //     setFormData({ ...formData, signers: newSigners })
                                            // }}
                                            onChange={(e) => handleSignerChange(e, index, 'address')}
                    ...
                    <div className='pb-6 border-b border-border-light px-0 md:px-8'>
                        ...
                        {errors.threshold && (
                            <p className="text-red-500 text-sm mt-4">* {errors.threshold}</p>
                        )}
                    </div >
                </>
            )
                ...
                <>
                    <div className='flex items-center justify-start gap-4 border-b border-border-light mb-8'>
                        ...
                        <div className='flex-1 flex flex-col'>
                            ...
                            <span className="text-gray-400 text-sm md:text-md mb-4">You're about to create a new Orbit Account and will have to confirm the transaction with your connected wallet..</span>
                        </div>
                    </div>
                    <div className='pb-6 border-b border-border-light px-0 md:px-8'>
                        <div className="space-y-6 mb-4">
                            ...
                            <div className="flex flex-col md:flex-row justify-start">
                                <span className="text-gray-400 w-1/4 mb-2 md:mb-0">Signers</span>
                                        ...
                                        return (
                                            <div className='flex items-center gap-2' key={index}>
                                                <Blockies
                                                    ...
                                                    scale={4}
                                                />
                                                <span className='break-all text-sm flex flex-col'><span className='text-gray-400'>{signer.name}</span>{signer.address}</span>
                                            </div>)
                                    }) : null}
                                </div>
    ...
    const handleCreateOrbitMultisig = async () => {
        ...
        const validSigners = formData.signers.filter(signer => signer.name.trim() !== '' && signer.address.trim() !== '');

        // Update local storage with valid signers only
        if (validSigners.length > 0) {
            const updatedSigners = validSigners.map(signer => {
                const existingSigner = LocalStoredsigners.find(s => s.address === signer.address);
                if (existingSigner) {
    ...
    const handleStepForward = (step: number) => {
        if (step === 2) {
            ...
            if (validSigners.length < 2) {
                errorMessages.signers = "At least 2 valid signers with addresses are required.";
            }
        ...
        <>
            <div className="min-h-screen bg-[#121212] text-white p-6 md:p-8 font-dmsans">
                <div className="max-w-6xl mx-auto">
                    <h1 className="text-3xl font-bold mb-8">Create Orbit Multisig Wallet</h1>
