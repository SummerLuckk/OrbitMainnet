...
export default function CalendarView() {
  ...
  const [selectedEvent, setSelectedEvent] =
    useState<Partial<CalendarEvent> | null>();
  ...
  const getWalletTransactions = async (walletAddress: Address) => {
    ...
    try {
      const transactionsData: Transaction[] | unknown =
        await walletContract.read.getAllTransactions();
      console.log(transactionsData);
      if (transactionsData) {
        setTransaction(transactionsData as Transaction[]);
      }
      const thresholdValue =
        await walletContract.read.numConfirmationsRequired();
  ...
  useEffect(() => {
    if (walletAddress) {
      getWalletTransactions(walletAddress as Address);
    }
  }, [walletAddress]);
  ...
  const fetchSignatures = async (txIndex: number) => {
    try {
      ...
      const response = await fetch(
        `/api/manage-signature?walletAddress=${walletAddress}&txIndex=${txIndex}`
      );
    ...
    try {
      if (
        ...
        tx = await writeContractAsync({
          ...
          args: [transaction.txIndex, signatures],
        });
      ...
      toast(
        "some error occured make sure your Orbit wallet has the required balance"
      );
      console.log("not enougn funds");
    }
  };
      ...
      >
        <DialogContent className="max-w-2xl bg-dark-gray text-white max-h-[90vh] overflow-y-auto p-10">
          ...
          <div className="py-4">
            ...
            {signerAddress.length > 0 && <p>{signerAddress.join(", ")}</p>}
          ...
          <DialogFooter>
            ...
            {!selectedEvent?.executed ? (
              <Button
                className="bg-accent text-black hover:bg-accent disabled:bg-gray-600 disabled:text-gray-400"
    ...
    <div className="flex flex-col md:flex-row justify-between items-center mb-4 text-white">
      <div className="flex space-x-2 mb-2 md:mb-0">
        <Button
          ...
          className="text-white bg-transparent border-[#2c2c2c] hover:bg-[#2c2c2c]"
        >
      ...
      <div className="flex space-x-2">
        <Button
          ...
          className="text-white bg-transparent border-border-light"
        >
          Today
        </Button>
        <Select onValueChange={(value) => onView(value as View)} value={view}>
          ...
          <SelectContent className="bg-dark-gray">
            <SelectItem value="day" className="bg-dark-gray text-accent">
              Day
            </SelectItem>
            <SelectItem value="month" className="bg-dark-gray text-accent">
              Month
            </SelectItem>
            <SelectItem value="week" className="bg-dark-gray text-accent">
              Weekly
            </SelectItem>
            <SelectItem value="agenda" className="bg-dark-gray text-accent">
              Agenda
            </SelectItem>
          </SelectContent>
        </Select>
