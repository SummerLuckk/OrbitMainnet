...
const TransactionRow = ({
  ...
  const addOwner = async () => {
    ...
    const response = await fetch(
      `/api/manage-settings/get-by-nonce?walletAddress=${userWallet.walletAddress}&nonce=${transaction.nonce}`
    );
    ...
    try {
      ...
      toast("New owner added successfully");
    } catch (error) {
  ...
  const updateThreshold = async () => {
    ...
    const response = await fetch(
      `/api/manage-settings/get-by-nonce?walletAddress=${walletAddress}&nonce=${transaction.nonce}`
    );
    const data = await response.json();
    // console.log(data);
    const signatures: any = data.signatures.signature;
    ...
    try {
      ...
      // Reset the input after successful transaction
      toast("Threshold updated successfully");
    } catch (error) {
      console.error("Error adding owner:", error);
    }
  };
      ...
      <TableCell>
        <Dialog>
          <DialogTrigger asChild>
            <Button
              className="bg-accent text-black hover:bg-accent hover:text-black border-accent"
              variant="outline"
              size="sm"
          ...
          <DialogContent className="text-white bg-dark-gray border-none p-0 m-0 rounded-lg overflow-hidden font-dmsans">
            ...
            <div className="p-3 md:p-6 space-y-6">
              <DialogHeader className="border-b border-border-light pb-4">
                <DialogTitle>
                  {txType === "owner" ? "Owner Update" : "Threshold Update"}
                </DialogTitle>
              </DialogHeader>
              <div className="grid gap-4">
                <div>
                  <p className="text-gray-400 text-sm">Wallet Address:</p>{" "}
                  <p className="text-[1rem] py-1">
                ...
                {txType === "owner" ? (
                  ...
                  <div>
                    <p className="text-gray-400 text-sm">New Threshold:</p>{" "}
                ...
                <div>
                  <p className="text-gray-400 text-sm">
                    ...
                    <span className="text-white">
                      {userWallet && userWallet.requiredSignatures} out of{" "}
                      {userWallet && userWallet.signerAddresses.length}{" "}
                      signer(s)
                    </span>
                  </p>
                  {userWallet && userWallet.signerWithName.length > 0
                    ? userWallet.signerWithName.map((signer, index) => {
                        return (
                          <div
                            className="flex flex-row w-full items-center gap-2 my-2"
                            key={index}
                          >
                            <Blockies
                            ...
                            <div className="flex flex-col">
                              <span className="break-all">
                                ...
                                <span>
                                  {address === signer.address ? (
                                    <span
                                      className={
                                        " ml-2 px-1 py-0 rounded-full text-xs font-semibold bg-accent text-black"
                                      }
                                    >
                                      You
                                    </span>
                              ...
                              <span className="break-all">
                                {truncateAddress(signer.address as Address)}{" "}
                              </span>
                            </div>
                            <div className="ml-auto">
                              <SignerStatusIcon
                                status={
                                  transaction.signerAddress.includes(
                                    signer.address
                                  )
                                    ? "completed"
                                    : "pending"
              ...
              <DialogFooter>
                <Button
                  ...
                  disabled={transaction.signerAddress.includes(
                    address as Address
                  )}
                >
                  {signLoading ? (
                ...
                <Button
                  ...
                  disabled={
                    userWallet &&
                    transaction.signerAddress.filter((signerAddress) =>
                      userWallet.signerWithName.some(
                        (signer) => signer.address === signerAddress
                      )
...
export default function WalletUpdateRequests({
  transactions,
  userWallet,
}: WalletUpdateRequestsProps) {
  return (
    <div className="bg-dark-gray  rounded-lg overflow-hidden font-dmsans">
      ...
      <div className="p-4 px-6 space-y-6 pt-6">
        <h1 className="text-2xl font-semibold border-b border-border-light pb-4">
          Update Requests
        </h1>
          ...
          <TabsContent value="owner">
            <ScrollArea className="w-full whitespace-nowrap rounded-md border border-border-light">
              <Table>
                ...
                <TableBody>
                  {transactions
                    .filter((t) => t.transactionType === "owner")
                    .map((transaction) => (
                      <TransactionRow
                        key={transaction._id}
          ...
          <TabsContent value="threshold">
            <ScrollArea className="w-full whitespace-nowrap rounded-md border border-border-light">
              <Table>
                ...
                <TableBody>
                  {transactions
                    .filter((t) => t.transactionType === "threshold")
                    .map((transaction) => (
                      <TransactionRow
                        key={transaction._id}
                        transaction={transaction}
