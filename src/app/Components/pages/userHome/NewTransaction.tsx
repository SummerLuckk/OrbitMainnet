...
export default function NewTransaction({ date }: NewTransactionProps) {
  ...
  const scheduleTransaction = async () => {
    if (!WalletContractAddress || !newTransaction.to || !newTransaction.amount)
      return;
    ...
    try {
      ...
      const amount = parseUnits(
        ...
        isERC20 ? Number(tokenDetails?.decimals) : 18
      );
      ...
      toast("Transaction scheduled sucessfully");
    } catch (error) {
      console.error("Error scheduling transaction:", error);
      toast("Error scheduling transaction");
    }
  };
        ...
        <div className="p-4 px-6 space-y-6 pt-6">
            ...
            >
              <Switch.Thumb
                className={`block w-[21px] h-[21px] ${
                  isERC20 ? "bg-accent" : "bg-white"
                } rounded-full shadow-md transform transition-transform duration-100 ease-in-out will-change-transform`}
                style={{
                  transform: isERC20 ? "translateX(19px)" : "translateX(2px)",
                }}
              />
            </Switch.Root>
          ...
          {tokenDetails && (
            <div className="mb-2">
              ...
              <p>
                Balance:{" "}
                {parseFloat(tokenDetails.balance.toString()) /
                  Math.pow(10, Number(tokenDetails.decimals))}
              </p>
            </div>
          )}
          ...
          <div>
            ...
            <input
              ...
              placeholder={`Amount (${
                isERC20 ? tokenDetails?.symbol || "Tokens" : "BTT"
              })`}
              className="bg-transparent border border-border-light p-4 w-full bg-[#2a2a2a] rounded-md p-2 text-white focus:outline-none focus:ring-1 focus:ring-accent"
            />
          </div>
